...
Definované funkce:
    DBP(level, *args, sep=' ', end='\n', file=None) -> None
    input(prompt: str) -> str
    refer_inputs() -> None
    first_max(max_chars: int, text: str) -> str
    indented_lines(start:str, txt:str) -> str
    start_pkg(level: int, name: str, doc: str = '', new_line=True) -> None
    start_mod(level: int, name: str) -> None
    stop_mod(level: int, name: str) -> None
    END(name: str) -> None
...
def DBP(level, *args, sep=' ', end='\n', file=None) -> None:
    ...
    když je počáteční argument větší než hodnota DBG.
    Ostatní argumenty odpovídají funkci """
    if DBG >= level:
...
def first_max(max_chars: int, text: str) -> str:
...
def start_mod(level: int, name: str) -> None:
    """Funkce vytiskne informaci o počátku zavádění modulu.
    """
    DBP(level, f'===== Modul {name} ===== START')


def stop_mod(level: int, name: str) -> None:
    """Funkce vytiskne informaci o ukončení zavádění modulu.
    """
    DBP(level, f'===== Modul {name} ===== STOP')
...
def prSE(level:int, sk:bool, caller:str, msg:str='') -> str:
    ...
    print(f'{prSE.indent}¤¤¤¤¤ {caller} - ' +
          f'{"START" if sk else "END"}' +
          f'{"" if msg == "" else f" - {msg}"}')
    if sk:
...
def prIndLim(text:str, data:object, container=True, limit:int=70) -> str:
    """Vytiskne zadaný text a pokusí se zalomit uživatelský podpis
    argumentu data na hranicích slov tak, aby délka řádku nepřesáhla
    zadaný limit. Vrátí zalomený text
    Mnemonika: print with indent and limit
    """
    prSE(3, 1, 'prIndLim', f'{text=}, {container=}, {limit=}')
    prIN(4, f'{data=}')
    ...
    def rest(words:list[str], start:int, length:int) -> int:
        ...
        while ...:
            ...
            if sum > length:
                return start
            lst.append(' ')
    indent = len(text) + container  # Posun o otevírací závorku kontejneru
    spaces = '\n'  +  indent * ' '  # Zarovnání levého okra dalších řádků
    words  = str(data).split()
    max    = limit - indent         # Počet znaků do konce řádku
    index  = 0
    prIN(0, (f'{text}{len(text)=}, {indent=}{spaces}'
             f'{container=}, {max=}, {len(words)=}'))
    while ...:
